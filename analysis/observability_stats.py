...
N_EPISODES = 200
EPISODE_SEEDS = range(50_000, 50_000 + N_EPISODES)
ACTION_RNG_SEED = 50_000
TOLERANCE = 0.005
OUT = Path(__file__).with_name("observability_stats.csv")

ENVIRONMENTS = (
    ("Empty-Random-6x6", "MiniGrid-Empty-Random-6x6-v0"),
    ("Empty-8x8", "MiniGrid-Empty-8x8-v0"),
...
def measure_environment(env_name: str, env_id: str):
    ...
    try:
        for seed in EPISODE_SEEDS:
            ...
            env.action_space.seed(int(action_rng.integers(0, 2**32, dtype=np.uint32)))
...
def main():
    rows = []
    for env_name, env_id in ENVIRONMENTS:
        measurements = measure_environment(env_name, env_id)
        for metric, (value, n_observations) in measurements.items():
            published = PUBLISHED.get((env_name, metric))
    ...
    empty_variance = next(
        ...
        if r["env"] == "Empty-8x8" and r["metric"] == "initial_phi_variance"
    )
    if empty_variance != 0.0:
        raise AssertionError(f"Empty-8x8 initial potential variance is {empty_variance}, not 0")
    visibility = {
        r["env"]: r["value"] for r in rows if r["metric"] == "goal_visibility"
    }
    if not (visibility["Empty-Random-6x6"] > visibility["Empty-8x8"]
            > visibility["FourRooms"]):
        raise AssertionError(f"unexpected visibility ordering: {visibility}")

    OUT.parent.mkdir(parents=True, exist_ok=True)
    with OUT.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=(
            "env", "metric", "value", "n_episodes", "n_observations",
            "published_value", "abs_diff", "agrees",
        ))
        writer.writeheader()
