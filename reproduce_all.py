...
def auxiliary():
    # The task-06 brief says these were measured, but its required analysis script and
    # raw observation/random-policy traces are absent. Preserve the values while making
    # their non-reproducible status explicit instead of silently presenting constants as
    # regenerated measurements.
    published = {
        "goal_visibility.Empty-Random-6x6": 0.562,
        "goal_visibility.Empty-8x8": 0.270,
        "goal_visibility.FourRooms": 0.094,
        "initial_phi_variance.Empty-8x8": 0.000,
        "initial_phi_variance.Empty-Random-6x6": 0.046,
        "initial_phi_variance.FourRooms": 0.053,
        "random_success.Empty-Random-6x6": 0.67,
        "random_success.FourRooms": 0.093,
    }
    for metric, value in published.items():
        add("auxiliary", metric, value, status="published_not_reproducible",
            source="codex-task-06.md / report documents; raw traces and analysis script absent")
...
def write_outputs():
    ...
    lines.extend(["", "## Reproducibility gaps and document differences", "",
                  ...
                  "- `analysis/observability_stats.py` and its CSV required by task-06 are absent. "
                  "Therefore goal visibility, initial-potential variance, and random-policy success "
                  "rates are published values, not regenerated values.",
